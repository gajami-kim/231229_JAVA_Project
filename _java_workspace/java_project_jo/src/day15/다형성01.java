 ...
 * - 각각의 객체는 메시지(리턴타입/매개변수)를 주고 받으면서 데이터를 처리
 * - 특징 : 추상화, 캡슐화(정보은닉), 상속, 다형성
 *
 * - 추상화(Abstract) : 핵심적인 코드만 보여주기 / 불필요한 부분은 숨긴다
 * 					  구현된 부분과 구현되지 않은 부분으로 분리함
 * - 캡슐화(Encapsulation) : 데이터보호(정보은닉) / 멤버변수(필드)는 숨기도 메서드로 접근
 * 						   멤버변수와 메서드를 하나로 묶어서 처리
 *						  - 은닉화:객체의 내부의 장보는 숨겨 외부로 드러나지 않게 하는 것
 *								 외부에서 데이터에 직접 접근하는 것을 방지
 * - 상속(Inheritance) : 코드 재사용(확장) / 유지보수가 편함
 * 					    클래스를 상속받아 수정하며 사용하면 중복코드를 줄일 수 있음
 * - 다형성(Polymorphism) : 객체 형변환 용이
 * 						  하나의 코드가 여러 자료(객체)형으로 구현되어 실행되는 것
 * 						  코드는 같은데 들어오는 객체에 따라 다른 실행결과를 얻을 수 있음
 * 						  다형성을 잘 활용하면, 유연하고 확장성있는, 유지보수가 편안 프로그램을 만들 수 있음					
 */
...
public class 다형성01 {
	...
	public static void main(String[] args) {
		...
		System.out.println("--조상의 객체로 자식의 객체 생성--");
		
		//자식의 객체로 부모를 생성하는건 안불가능
//		Human h1 = new Animal(); //불가능

		//조상의 클래스로 자식의 객체 생성이 가능
		//조상의 클래스에서 상속받은 것만 실행가능
		Animal hAnimal = new Human(); 
		hAnimal.move();
//		hAnimal.readBook() //자식자체의 것(상속x)이기 때문에 실행 불가능
...
//		ex1.moveAnimal3(e);
		...
		Dog d = new Dog();
		d.move();
//		ex1.moveAnimal(d); //Animal을 상속받고 있지 않기 때문에 불가능
		
		System.out.println();
		System.out.println("--Animal 배열생성성 예시--");
		...
		//Human을 담을 수 있는 자료형의 객체에 => Animal을 상속받은 Human의 클래스 객체를 담은 케이스
		Human human = (Human)hAnimal;
//		Human human = (Human)tAnimal; //다운캐스팅 오류
		human.readBook();
		
		//error 발생 케이스 *ClassCastException
		//다운캐스팅의 잘못된 예시, 조상>>자식 형태가 아니기 때문
//		Human human1 = (Human)tAnimal;
//		human1.readBook();
		
		//instanceof : 객체가 해당 클래스의 형에 맞는지 체크하는 연산자
		System.out.println(tAnimal instanceof Human); //false
		System.out.println(tAnimal instanceof Tiger); //true
		System.out.println(tAnimal instanceof Eagle); //false
