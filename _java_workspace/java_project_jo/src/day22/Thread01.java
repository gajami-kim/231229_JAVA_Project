...
public class Thread01 {
	...
	public static void main(String[] args) {
		/* Thread(쓰레드)
		 * - 실행중인 프로그램이 OS로부터 메모리를 할당받아 process 상태가 됨
		 * - 하나의 process는 하나 이상의 thread를 가지게 되고,
		 * - thread는 실제 작업을 수행하는 단위를 말한다
		 * - Thread란 실제 CPU가 작업을 처리하는 단위
		 *
		 * - multi-thread : 여러개의 thread가 동시에 수행되는 프로그램
		 * - CPU는 시간을 잘게 쪼개서 thread를 번갈아 처리
		 * - 사용자들은 동시에 처리되는듯한 효과를 가지게 됨
		 * 
		 * - thread는 각각 자신만의 작업 공간을 가짐(context)
		 * - 각 thread는 공유하는 자원이 있을 수 있음(자바-static instance)
		...
		//자바는 다중상속을 허용하지 않기 때문에, 다른 클래스를 상속중인 경우는 2번으로 구현 / 아니라면 보통 1번을 사용
